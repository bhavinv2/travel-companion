...
// Rename a saved view. One dialog for the page; the button that opened it says which view and
// what it is currently called, so the field starts from the existing name rather than blank.
(function () {
  const dlg = document.getElementById('svRenameDlg');
  if (!dlg) return;
  const form = document.getElementById('svRenameForm');
  const input = document.getElementById('svRenameInput');
  const close = () => { dlg.hidden = true; };

  document.addEventListener('click', e => {
    const btn = e.target.closest('[data-sv-rename]');
    if (btn) {
      // the URL is generated by Jinja and carries the app's prefix; building it here would
      // 404 on the subpath deployment
      form.action = btn.dataset.svRename;
      input.value = btn.dataset.svName || '';
      dlg.hidden = false;
      setTimeout(() => { input.focus(); input.select(); }, 40);
      return;
    }
    if (e.target.closest('#svRenameCancel') || e.target === dlg) close();
  });
